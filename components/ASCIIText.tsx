...
class CanvAscii {
  ...
  onMouseMove(evt: MouseEvent | TouchEvent) {
    const e = "touches" in evt && evt.touches ? evt.touches[0] : (evt as MouseEvent);
  ...
  updateRotation() {
    if (!this.mesh) return;
    const x = mapNumber(this.mouse.y, 0, this.height, 0.5, -0.5);
    const y = mapNumber(this.mouse.x, 0, this.width, -0.5, 0.5);
